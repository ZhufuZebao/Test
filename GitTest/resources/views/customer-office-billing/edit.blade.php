...
@section('header')
    <title>{{ $model->id ? "編集" : "新規作成" }} | 請求先 | {{ array_get($model,'office.name') }} | 顧客 | {{ config('app.name') }}</title>
@endsection
...
@section('content')
		...
		<header>
			...
			<ul class="header-nav friend">
                ...
                @if($model->id)
				    <li class="current"><a href="{{ route('customer-billing.edit',['bid'=>$model->id]) }}">編集</a></li>
                @else
				    <li class="current"><a href="{{ route('customer-billing.create',['bid'=>$model->id]) }}">新規</a></li>
                @endif

...
<div class="container low user">
    ...
    <form class="form-horizontal" name="input_form" role="form" method="POST" action="{{ url('/modconfirm') }}">
    {{ csrf_field() }}
    <input type="hidden" name="id" id="id" value="{{ $id }}" />
    <input type="hidden" name="backpage" value="usermodify" />
...
*/ ?>
        ...
        <dt>誕生日</dt>
        <dd><input type="date" name="birth" id="birth" value="{{ old('birth', $user->birth) }}" placeholder="例）1980/01/01"></dd>
...
*/ ?>
        ...
        <dd>
            <input type="text" name="zip" id="zip" value="{{ old('zip', $user->zip) }}" placeholder="例）530-0001" style="ime-mode:disabled;">
        ...
        <dd>
            ...
            @foreach($prefs as $index => $name)
                <option value="{{ $index }}" @if(old('pref', $user->pref) == $index || $pref == $index) selected @endif>{{ $name }}</option>
            @endforeach
            </select>
        </dd>
        ...
        <dt>それ以降の住所</dt>
        <dd>
            <input type="text" name="addr" id="addr" value="{{ $addr != '' ? $addr : old('addr', $user->addr) }}" style="ime-mode:active;">
        ...
        <dt>会社名・団体名</dt>
        <dd>
            <input type="text" name="comp" id="comp" value="{{ old('comp', $user->comp) }}" style="ime-mode:active;">
        ...
        <dt>部署名・役職</dt>
        <dd>
            <input type="text" name="class" id="class" value="{{ old('class', $user->class) }}" style="ime-mode:active;">
...
<script>
function searchAddr() {
    document.input_form.action = '/shokunin/searchAddr2';

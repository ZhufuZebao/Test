...
@section('header')
    ...
    <title>{{ $model->name }}|工程|{{ config('app.name') }}</title>
...
@section('content')
	...
	<section class="process-wrapper">
        ...
        <a href="{{ route('task.edit',['id'=>$model->id]) }}">編集</a>

        {!! Form::open(['url'=>route('task.pdf',['id'=>$model->id]),'method'=>'post','id'=>'form-generate-pdf','target'=>'_blank']) !!}
        {!! Form::hidden('base64', null, ['id'=>'input-hidden-base64']) !!}
        <button onclick="outputPdf()">PDF</button>
        {!! Form::close() !!}

        {!! Form::open(['url'=>route('task.excel',['id'=>$model->id]),'method'=>'post','id'=>'form-generate-excel','target'=>'_blank']) !!}
...
@section('footer')
    ...
    <script>
	 var func = function (e) {
		 ...
		 var value = el.value;
		 g2.config = setScaleConfig(value);// see also dhtmlxgantt-locale-ja.js
         g2.render();
	 };

<template>
  <div class="ChatContactPage">
    <el-tabs class="common-view chat-only" :tab-position="tabPosition" v-model="linkGroupCurrent">
      <div class="chatpersonContent" id="parentGroup">
        <el-row class="common-view">
          <el-col :span="24">
            <el-menu
                    ref="parentGroup"
                    background-color="#e5e5e5"
                    :unique-opened="false">
              <el-submenu :index="g.id+''" v-for="(g,gIndex) in chatPerson" :key="'group-'+g.name+gIndex">
                <template slot="title">
                  <div  class="chat-group" style="width: 95%;">
                  ...
                  <span class="chat-msg-num" v-if="getNum(g)">
                     {{getNum(g)}}
                   </span>
                  <span v-else>
                ...
                <el-menu-item v-for="(c,index) in g.child" :key="'child-'+c.id+index"
                              :index="c.id+''" v-if="c" class="chat-group">
                  <img :src="'file/groups/'+c.file" alt="" class="pro-photo" v-if="c.file">
                  <img src="images/icon-chatgroup.png" alt="" class="pro-photo" v-else>
                  <span class="chat-group-name" v-if="c.name" @click.prevent="showRightModel(c,c.id)">{{c.name}}</span>
                  <span class="chat-msg-num" v-if="uc.group_id===c.id && uc.num>0" v-for="uc in UnreadCount"
                        :key="'UnreadCount-'+uc.id">
                     {{uc.num}}
                   </span>
                </el-menu-item>
                <!--<el-menu-item :index="g.id+'add'" v-if="member.admin">-->
                <el-menu-item :index="g.id+'add'" v-if="getGroupAdmin(g)">
                  <el-popover placement="right-start" width="300" trigger="click"
                              :ref="'pochild-' + g.id" popper-class="popover-location">
                    <div class="popover-div">
                      ...
                      <el-form ref="forms" :model="chatSetGroup" :rules="chatGroup">
                        <el-form-item prop="inputPopover" class="chatListadd">
                          <el-input v-model="chatSetGroup.inputPopover" placeholder="名前" maxlength="191"
                                    class="popoverto-input" onkeypress="if (event.keyCode === 13) return false;">
                          </el-input>
                        </el-form-item>
                      </el-form>
                      <li class="popover-li">メンバーを選択</li>
                      <el-tabs type="border-card" class="popoverto-tabs content-popover">
                        <el-input class="popoverto-input" placeholder="名前で検索"
                                  v-model="searchChildGroupPerson"
                                  suffix-icon="searchForm-submit"></el-input>

                        <el-tab-pane label="社内">
                          <div class="popover-checkbox" v-if="searchChildGroupPerson">
                      ...
                      <li class="popover-li">
                        <el-button class="group-creat" @click="setGroup(0,g.id)">
                          グループチャット作成
                        </el-button>
                      </li>
                    </div>
                    <div class="chat-add" slot="reference" @click.prevent="showCreatChildGroup(g.id)">
      ...
      <section class="chatcontent" v-on:mouseleave="mouseleave" v-if="showChatFlag">
        <ul class="rowfrist">
          <div class="talk-user-item-wrap clearfix" style="align-items: center;display: flex;" :class="[{'showjoinms':showJoinMsg && (showAudioType || showVideoType)}]">
            <img style="cursor: pointer;" class="talk-photo" v-bind:src="'file/groups/' + member.photo" @click="showPersonDetail"
                 v-if="member.photo && member.photo!=='null'">
            <img style="cursor: pointer;" class="talk-photo" src="images/icon-chatgroup.png" @click="showPersonDetail" v-else>
            <div style="cursor: pointer;" class="talk-name" @click="showPersonDetail">{{member.name}}</div>
            <el-popover placement="right-start" popper-class="popover-location" width="300" trigger="click"
              ...
              <div class="popover-div">
                <li class="popover-li">
                  <el-row>
                    <img v-if="imageUrl" v-bind:src="imageUrl">
                    <img v-else-if="this.member.photo"
                         :src="'file/groups/' + member.photo">
                    <img v-else src="images/icon-chatgroup.png">
                    <div class="el-form-item__error" v-if="validateMessage">
                      {{validateMessage}}
                    </div>
                ...
                <li class="popover-li">
                  <el-button class="group-creat" @click="saveGroup()">保存
                  </el-button>
                </li>
              </div>
              <div class="talk-name edit-photo chatlist-newicon" slot="reference"
                   @click="editGroup">
                <img src="images/edit@2x.png" alt="" v-if="member.parentId"/>
              </div>
            </el-popover>
            <div class="talk-contents" style="margin-left: 35px">{{member.len}}人</div>
          </div>
          <p>
            <span v-if="showJoinMsg">
            ...
            <el-popover placement="left" v-model="showVideoTabFlag" class="chat-model" :class="[isTransverse?'is-transverse':'no-transverse']">
              <div class="chat-popover" :class="[isFullscreen?'is-fullscreen':'no-fullscreen']">
                <div class="popover-header">{{member.name+' '+member.enterpriseName+'('+(videoArr.length+1)+'/'+member.len+')'}}</div>
                  ...
                  <el-popover popper-class="model-joiner"
                    ...
                    <div>
                      <li class="chat-model-joiner-li" v-for="u in joinerList(videoArr)">
                        <div v-if="u.file"><img :src="'file/users/'+u.file"><span>{{u.name}}</span></div>
                        <div v-else><img src="images/icon-chatperson.png"><span>{{u.name}}</span></div>
                      </li>
                      <!--person data add-->
                ...
                <div class="popover-tabs-content">
                  <div class="popover-content-img">
                    <div v-show="showVideoFlag">
                      ...
                      <div class="imgguests">
                        <video id="my-video-proj" autoplay playsinline class="imghost"></video>
                      </div>
                    </div>
            ...
            <el-popover popper-class="group-popover" v-model="groupMemberShowFlag">
              <div class="group-person">
                        ...
                        <el-checkbox-group v-model="enterpriseArr">
                          <li v-for="user in friendArr.enterpriseArr" :key="'enterpriseArr-'+user.id"
                            ...
                            <el-checkbox :label="user">
                              ...
                              <img :src="'file/users/'+ user.file" alt="" v-else-if="member.id!==''">
                              {{user.name}}
                            </el-checkbox>
                          </li>
                        </el-checkbox-group>
                ...
                <div class="group-person-add">
                  <el-button @click.prevent="searchFriend(1)" v-if="member.admin">+</el-button>
                  <div>
                    <ul class="group-person-box" style="height:180px;overflow-x: auto">
                      <li v-for="(user,index) in groupUsers" :key="'groupUsers'+index">
                        ...
                        <img :src="'file/users/' + user.user.file" alt="" v-else-if="member.id!=='' && user.user">
                        <span class="el-icon-error"
                              v-if="mineId!==user.user.id"
                              @click.prevent="delInGroup(user,index)"></span>
                        <div>{{user.user.name}}</div>
                      </li>
                    </ul>
                    <el-button @click="closeGroupPopover">閉じる</el-button>
                    <el-button v-if="member.admin" @click="clearGroup">グループを解散</el-button>
                  </div>
                </div>
              </div>
            </el-popover>
            <el-button @click.prevent="showGroupUser(1)" class="chat-memeber-add"
                       v-if="member.admin && member.parentId">+
            </el-button>
          </p>
        </ul>
        <ul class="rowsecond" id="chatList" ref="chatLists"  :style="{height: 'calc( 100% - 244px - ' + (updated_filecount)*36+'px)' }">
          <li v-for="(chatItem,index) in chatLists" v-on:mouseover="mouseover(index)" v-if="chatItem"
            ...
            <h4 style="align-items: center;display: flex;" class="talk-user-item-wrap-contact">
              <el-popover placement="left" ref="chatToUsr">
                <div>
                  ...
                  <el-button @click="toChatList(chatItem)"
                             v-else-if="user.id!==chatItem.from_user_id &&chatItem.del!=='delete'">
                    チャットする
                  </el-button>
                </div>
                <img class="chat-avatar talk-photo" slot="reference" src="images/icon-chatperson.png"
                     v-if="chatItem.user && !chatItem.user.file">
                <img class="chat-avatar talk-photo" slot="reference" v-bind:src="'file/users/' + chatItem.user.file"
                     v-else-if="member.id!==''">
              </el-popover>
            ...
            <div class="chat-contants-wrap add-chat-contants-wrap">
              <div class="talk-item-wrap clearfix">
                <el-button type="primary" class="el-button--primary-left">
                      ...
                      <el-tabs type="border-card" class="popoverto-tabs">
                        ...
                        <div class="toChoice">
                          <p v-for="(groupUser,index) in groupUsers" class="center-style"
                            ...
                            <img :src="'file/users/' + groupUser.user.file" alt="" v-else-if="member.id!==''"
                                 style="height: 30px">
                  ...
                  <el-button @click="selectFilesFromDoc" type="primary"><img src="img/icon/doc.png" /></el-button>
                  Ctrl + Enterで送信
                </el-button>
    ...
    <ChatFileList v-if="showChatFileListFlag" @closeFileShow="closeFileListShow"
                  :groupId="member.id"></ChatFileList>
    <ChatListTask v-if="showTaskFlag" :taskGroupUser="groupUsers" :chatClickMessage="clickMessage"
                  @taskBack="taskCancel" :groupId="member.id" :chatJumpIdFn="chatJumpId"></ChatListTask>
    ...
    <ChatPersonList v-if="showPersonDetailModal" @closePersonDetailModel="closePersonDetailModel"
                  :member="member" ref="ChatPersonListModel" :groupId="member.id"></ChatPersonList>
...
<script>
  ...
  export default {
    name: "ChatContact",
    components: {ChatFileList, ChatListTask, MailModal, ChatPersonList, FileSelector},
    mixins: [
      reportValidation,
      messages,
      formatChatMsg,
    ...
    props: ['chatMessageSearchWord', 'searchChatListFlag', 'leaveToChatProFlag', 'chatJumpObj', 'activeFlag'],
    data: function () {
      return {
        isTransverse :false,
        updated_filecount: '',
    ...
    methods: {
      ...
      getNum(group){
        ...
        for (let i=0;i<group.child.length;i++){
          item.push(group.child[i].id);
        }
        for (let i=0;i<this.UnreadCount.length;i++){
          if (item.indexOf(this.UnreadCount[i].group_id)>=0){
            num += this.UnreadCount[i].num;
          }
        }
        if (num > 0){
          return num;
      ...
      toChatList(item){
        ...
        for (let i=0;i<this.chatLists.length;i++){
          this.$refs['chatToUsr'][i].doClose();
        }
        axios.post('/api/getGroup', {
          userId:item.from_user_id
        }).then((res) => {
          groupId = res.data;
          this.$emit('setChatList',groupId,0,'linkGroup-'+groupId);
        }).catch(err => {
          loading.close();
      ...
      getGroupAdmin(data){
        ...
        data.users.filter(item => {
          if (item.id === this.mineId && item.pivot.admin === 1) {
            adminGroup =  true;
          }
        });
        return adminGroup;
      ...
      getPermit(data) {
        ...
        data.users.filter(item => {
          if (item.id === this.mineId && item.pivot.admin === 1) {
            this.member.admin = true;
          }
        });
      },
      showCreatChildGroup(id) {
        ...
        this.searchChildGroupPerson = null;
        const loading = this.$loading({lock: true, background: 'rgba(0, 0, 0, 0.7)'});
        let errMessage = this.commonMessage.error.insert;
        axios.post('/api/getGroupContact', {
          'groupId': id
        }).then((res) => {
          this.searchPerson.searchEnterprise = res.data.enterpriseArr;
          this.searchPerson.searchParticipant = res.data.participantsArr;
          this.searchPerson.searchGroup = res.data.contactArr;
        }).then(res => {
          loading.close();
        }).catch(err => {
          loading.close();
          ...
          if (valid && !this.validateMessage) {
            ...
            axios.post(url, data, headers).then(res => {
              ...
              if (res.data.result === 0) {
                this.member.name = this.editGroupName.inputPopover;
                if (res.data.params.img) {
                  this.member.photo = res.data.params.img + "?" + Date.now();
                }
                for (let data in this.chatPerson) {
                  let item = this.chatPerson[data]['child'];
                  this.chatPerson[data]['child'] = item.filter(data => {
                    if (data.id === this.member.id) {
                      if (this.member.photo) {
                        data.file = this.member.photo;
                      }
                      data.name = this.editGroupName.inputPopover;
                    }
                    return data;
                  });
                }
      ...
      editGroup() {
        ...
        this.validateMessage = '';
        this.imageUrl = null;
        this.editGroupName.inputPopover = this.member.name;
      },
      showImg(chatItem) {
        let msg = '';
        if (chatItem.file_name) {
          ...
          msg = formatChatMsg.formatImg(file_name, this.member.id);
        }
      ...
      showMessage(chatItem, type) {
        if (chatItem.message) {
          ...
          if (chatItem.message.indexOf('[time:') !== -1) {
            let msg = formatChatMsg.quote(this.HTMLEncode(chatItem.message),chatItem.file_name,this.member.id);
            return this.emoji(msg);
          } else {
            let msg = null;
            if (chatItem.message.indexOf('[To:') !== -1) {
              chatItem.message = formatChatMsg.formatToMsg(chatItem.message, this.member.id);
            }
            if (chatItem.message.indexOf('[mid:') !== -1) {
              msg = formatChatMsg.formatReMsg(chatItem.message, this.member.id);
              return this.emoji(msg);
            } else {
      ...
      jumpToChatMsg() {
        ...
        this.$refs.parentGroup.activeIndex = (this.member.groupId+"");
        if (!this.timer) {
          this.timer = true;
          setTimeout(() => {
            this.timer = false;
      ...
      mouseleave() {
        //デフォーカス
        document.getElementById('msgTextareaChatContact').blur();
      ...
      showVideoTab(type,linkFlag = 0) {
        ...
        this.getUserMedia(type).then(res => {
          if (type === 1) {
            ...
            if (this.startVideoGroupIdArr.indexOf(this.member.id) === -1) {
              if (!linkFlag) {
                this.sendMsg = '[icon:101]';
                this.sendMessage(1);
              }
              // let mineRoomName = "group_audio_" + this.member.id;
            ...
            if (this.startVideoGroupIdArr.indexOf(this.member.id) === -1) {
              ...
              // let mineRoomName = "group_video_" + this.member.id;
              // this.joinVideo(mineRoomName);
      ...
      uploadVideoPic(file) {
        ...
        data.append("groupId", this.member.id);
        axios.post("/api/setVideoPic", data, headers).then(res => {
          if (res.data.result === 0) {
            if (res.data.params) {
              this.videoPicName = 'file/' + this.member.id + '/' + encodeURIComponent(res.data.params);
              this.hasShowFile = false;
              this.downloadChatFile = this.member.id + '/' + encodeURIComponent(res.data.params);
              let msg = {
        ...
        axios.post("/api/setVideoPdf", data, headers).then(res => {
          if (res.data.result === 0) {
            if (res.data.params) {
              ...
              this.videoPicName = 'file/' + this.member.id + '/' + encodeURIComponent(res.data.params.imagePath);
              this.hasShowFile = false;
              this.downloadChatFile = this.member.id + '/' + encodeURIComponent(this.pdfFileName);
      ...
      changeClickChatItem(group) {
        this.fetchChatPersonList(group.pro);
        this.showChatFlag = true;
        this.$refs.parentGroup.activeIndex = (group.pro+"");
      },
      ...
      join: function () {
        this.user.room = 'group_' + this.member.id;
        this.socket.emit("join", this.user);
      },
      leave: function () {
        this.socket.emit("leave", this.user.room);
      ...
      clearGroup() {
        const loading = this.$loading({lock: true, background: 'rgba(0, 0, 0, 0.7)'});
        this.$confirm(this.commonMessage.confirm.delete.group(this.member.name),
            {
          ...
          axios.post("/api/clearGroup", {
            ...
            this.delPersonComment(this.member.id);
            loading.close();
          }).catch(err => {
            console.warn(err);
            loading.close();
        ...
        if (data === 1 || (event.ctrlKey && event.keyCode === 13)) {
          ...
          if (this.sendMsg.replace(/\n*$/g, '').replace(/\n/g, '') === '' && !this.fileArr.length) {
            ...
            if (this.updateMsgFlag) {
              ...
              let data = new FormData();
              let headers = {headers: {"Content-Type": "multipart/form-data"}};
              data.append('group_id', JSON.stringify(this.member.id));
              ...
              axios.post("/api/setChatMessage", data, headers)
                  .then(res => {
                    if (res.data.result === 0) {
                      window.messageNewProGroup = this.member.id;
                      //#2645 turn url into hyperLinks
      ...
      sendRemoveUserMessage(removeId, name) {
        ...
        let headers = {headers: {"Content-Type": "multipart/form-data"}};
        dataFrom.append('group_id', JSON.stringify(this.member.id));
      ...
      toUser(user) {
        ...
        if (this.sendMsg === '') {
          ...
          //マウスカーソルに挿入'[To]'
          this.msgCursortPositionChatContact = $('#msgTextareaChatContact')[0].selectionStart;
          this.sendMsg = this.insertStr(this.sendMsg,this.msgCursortPositionChatContact,'[To]' + user.name)
        }
      ...
      toAllUser() {
        ...
        if (this.sendMsg === '') {
          ...
          //マウスカーソルに挿入'[To]'
          this.msgCursortPositionChatContact = $('#msgTextareaChatContact')[0].selectionStart;
          this.sendMsg = this.insertStr(this.sendMsg,this.msgCursortPositionChatContact,'[To]All');
        }
      ...
      addFriendToGroup() {
        ...
        for (let i = 0; i < this.enterpriseArr.length; i++) {
          ...
          this.enterpriseArr[i].group_id = this.member.id;
        ...
        for (let i = 0; i < this.contactArr.length; i++) {
          ...
          this.contactArr[i].group_id = this.member.id;
        ...
        for (let i = 0; i < this.parterArr.length; i++) {
          ...
          this.parterArr[i].group_id = this.member.id;
      ...
      showGroupUser(index) {
        ...
        if (this.groupUsers.length > 0) {
          if (index === 1 && this.groupUsers[0].group_id === this.member.id) {
            return;
          }
        }
        let headers = {headers: {"Content-Type": "multipart/form-data"}};
      ...
      receiveMemberMsg(groupId, parentId) {
        if(this.activeFlag !== 'activeThird'){
          return;
        }
        let errMessage = this.commonMessage.error.chatList;
        const loading = this.$loading({lock: true, background: 'rgba(0, 0, 0, 0.7)'});
      ...
      handleScroll() {
        ...
        if(document.getElementById('chatList').scrollTop > this.showPosition){
          return;
        }

        //trigger when reaching the specified height
        if((document.getElementById('chatList').scrollTop < document.getElementById("li"+this.scrollId).scrollHeight) && !this.nextPage){
          document.getElementById('chatList').removeEventListener('scroll', this.handleScroll);
          this.showPosition = document.getElementById("chatList").scrollHeight;
          this.nextPage=true;
          let errMessage = this.commonMessage.error.chatList;
            ...
            if (res.data.result === 0) {
              ...
              if(res.data.params.models.length > 0) {
                ...
                if(this.chatLists.length > 0){
                  ...
                  document.getElementById('chatList').addEventListener('scroll', this.handleScrollx);
                }
                if (this.chatJumpObj) {
                  this.jumpToChatMsg();
                }
      ...
      fetchChatPersonList(groupId = 0) {
        ...
        let errMessage = this.commonMessage.error.insert;
        let url = "/api/getProjGroupsChatList";
        let data = new FormData();
        let headers = {headers: {"Content-Type": "multipart/form-data"}};
        axios.post(url, data, headers).then(res => {
          this.chatPerson = res.data.params.group;
          this.UnreadCount = res.data.params.lastReadMsg;
          if (groupId) {
            for (let data in this.chatPerson) {
              let item = this.chatPerson[data]['child'];
              item.filter(data => {
                if (data.id == groupId) {
                  this.showRightModel(data,data.id)
                }
              });
            }
            this.$refs.parentGroup.activeIndex = (groupId+"");
          } else {
            this.current = null;
            this.showChatFlag = false;
            this.linkGroupCurrent = null;
            this.$refs.parentGroup.activeIndex = "";
              if (this.$refs.parentGroup) {
                  Object.values(this.chatPerson).forEach(item => {
                      this.$refs.parentGroup.close(item.id+"");
                      Object.values(item.child).forEach(value => {
                          this.$refs.parentGroup.close(value.id+"");
                      });
                  });
              }
          }
      ...
      fetchMember(data, type) {
        ...
        this.showChatFlag = false;
        $("#tab-linkGroup-" + data.id).addClass("is-active");
        if (this.member.id) {
          $("#tab-linkGroup-" + this.member.id).removeClass("is-active");
        }
        this.clearMessage();
        ...
        this.member.photo = data.file;
        this.member.enterpriseName = "";
        this.member.id = data.id;
        this.member.kind = 0;
        this.member.parentId = data.parent_id;
        if (data.parent_name){
          this.member.parentName = data.parent_name;
        }
        this.member.len = data.users.length;
        this.groupUserDatas = data.users;
        this.receiveMemberMsg(data.id, parentId);
      },
      //空かどうかを確認
      ...
      handleChange(file, fileList) {
        ...
        if (fileList.length) {
          ...
          data.append('group_id', this.member.id);
      ...
      delPersonComment(groupId) {
        if (groupId) {
          for (let i = 0; i < this.chatPerson.length; i++) {
            this.chatPerson[i].child = this.chatPerson[i].child.filter(item => {
              if (item.id.toString() !== groupId.toString()) {
                return item;
              }
            });
          }
      ...
      showRightModel(g, index) {
        //ポップアップイベント
        if (!this.visibled) this.visibled = true;
        this.current = index;
        this.fetchMember(g, 1);
        // if (this.$refs.parentGroup) {
        //   if (g.parentId) {
        //     this.$refs.parentGroup.open(g.parentId);
        //   } else if (g.parent_id) {
        //     this.$refs.parentGroup.open(g.parent_id);
        //   } else {
        //     this.$refs.parentGroup.open(index);
        //   }
        // }
        this.getPermit(g);
      },
      ...
      addPerson(group, $selector) {
        for (let i=0;i<this.chatPerson.length;i++){
          if (this.chatPerson[i].id === group.parent_id) {
            this.chatPerson[i].child.push(group);
          }
        }
        this.$refs.parentGroup.activeIndex = (group.id+"");
        this.showRightModel(group,group.id)
      },
      //ユーザー検索
      ...
      doConnect() {
        ...
        if (socketUri) {
          ...
          this.socket.on('chat.message', function (chatMsg) {
            ...
            if(self.activeFlag !== 'activeThird'){
              return;
            }
            ...
            if (msgObj.from_group_id && msgObj.from_group_id === self.member.id) {
              return false;
            }
            ...
            if (msgObj.removeId && msgObj.removeId === self.userSelf.id) {
              ...
              self.deleteGroupUnused(self.member.id);
            ...
            if (msgObj.mid === 0) {
              ...
              if (msgObj.vedio) {
                if (self.startVideoGroupIdArr.indexOf(self.member.id) === -1) {
                  self.startVideoGroupIdArr.push(self.member.id);
                }
              }
              let imgName = null;
              if (msgObj.file) {
          ...
          this.socket.on('chat.file', function (chatMsg) {
            ...
            self.videoPicName = 'file/' + self.member.id + '/' + encodeURIComponent(msgObj.file);
            self.downloadChatFile = self.member.id + '/' + encodeURIComponent(msgObj.file);
            if (msgObj.pdfFile) {
              self.downloadChatFile = self.member.id + '/' + encodeURIComponent(msgObj.pdfFile);
            }
          ...
          this.socket.on('chat.users', function (nicknames) {
            ...
            for (let i = 0;i < Object.keys(nicknames).length;i++){
              if(Object.keys(nicknames)[i] !== self.mineId &&
          ...
          this.socket.on('chat.deleteUser', function (chatMsg) {
            ...
            for (let i = 0; i < length; i++) {
              if (self.chatPerson[i].group_id === self.member.id) {
      ...
      videoConn(type, linkFlag = 0) {
        ...
        let peerId = 'peerID_' + this.member.id + '_' + this.mineId + '_' + this.getUuid();
      ...
      pushCall: function (status) {
        let data = {group_id: this.member.id, call_status: status};
      ...
      joinRoom: function () {
        ...
        this.room = this.peer.joinRoom(this.roomName + this.member.id, {
          mode: 'mesh',
      ...
      leaveRoom(type) {
        if (type === 2) {
          this.startVideoFlag = false;
          let index = this.startVideoGroupIdArr.indexOf(this.member.id);
          if (index > -1) {
        ...
        this.pdfPageNext = false;
        this.pdfPagePrev = false;
        this.socket.emit("chat.call.hangup", this.user.id); //client must emit call event,not receive event
      ...
      getUserMedia(type) {
        return new Promise((resolve, reject) => {
          ...
          Vue.nextTick(() => {
            ...
            let localVideo = document.getElementById('my-video-proj');
      ...
      deleteGroupUnused(groupId) {
        for (let i = 0; i < this.chatPerson.length; i++) {
          if (this.chatPerson[i]['id'] === groupId) {
      ...
      checkMessageUpdate() {
        ...
        if (this.member && this.member.id) {
          window.chatGroupNowContact = this.member.id;
        } else {
          window.chatGroupNowContact = 0;
        }
        if (this.member && this.member.id && window.messageNewProGroup == this.member.id) {
          this.moveItem2ArrFirst(this.chatPerson,window.messageNewProGroup);
          window.messageNewProGroup = 0;
        }
        //check unread messages
        if (window.messageNewProGroup > 0) {
          //change this group's unread number
          let UnreadIndex = this.UnreadCount.findIndex(function (item) {
            return item.group_id === window.messageNewProGroup;
          });
          if (UnreadIndex >= 0) {
            this.UnreadCount[UnreadIndex].num = this.UnreadCount[UnreadIndex].num + 1;
          } else {
            let tmp = {group_id: window.messageNewProGroup, num: parseInt(1)};
      ...
      moveItem2ArrFirst(arr,key){
        if (arr){
          ...
          for (let i = 0; i < arr.length; i++) {
            let item = arr[i].child;
            for (let j = 0; j < item.length; j++) {
              if (item[j].id.toString() === key.toString()) {
                let array = item[j];
                item.splice(j, 1);
                item.unshift(array);
                flag = true;
        ...
        document.getElementById("parentGroup").scrollTop = 0;
      },
      joinerList(videoArr){
        ...
        let groupUsersArray = this.groupUserDatas;
        for (let e = 0;e<groupUsersArray.length;e++){
          for (let i = 0;i<userIdRes.length;i++) {
            if (userIdRes[i] == groupUsersArray[e].id){
    ...
    computed: {
      //あいまい検索
      searchEnterpriseRes() {
        if (this.searchChildGroupPerson) {
          let search = this.searchChildGroupPerson.toLowerCase();
    ...
    watch: {
        ...
        if (window.videoChat && !window.rejectFlag && newMsg && newMsg.length) {
          if (!this.showVideoTabFlag) {
            for (let data in this.chatPerson) {
              let item = this.chatPerson[data]['child'];
              item.filter(data => {
                if (data.id == window.videoChat.group_id) {
                  this.showRightModel(data,data.id)
                }
              });
            }
            this.$refs.parentGroup.activeIndex = (window.videoChat.group_id + "");
          }
          ...
          if (window.videoChat.call_status === 'reject') {
            ...
            if (window.videoChat.group_id === this.member.id) {
              console.log('flag reject');
              //拒否人数+1
              this.rejectCount++;
      ...
      leaveToChatProFlag() {
        if (this.chatJumpObj) {
          this.changeClickChatItem(this.chatJumpObj);
        }
      },
        ...
        this.$nextTick(() => {
          if (document.getElementById("videoImage")) {
            if (this.showChatFlag) {
              if(this.setId){
                document.getElementById("chatList").scrollTop = document.getElementById("chatList").scrollHeight - this.showPosition;
              }else{
                document.getElementById('chatList').addEventListener('scroll', this.handleScroll);
            ...
            if (window.videoChat && window.videoChat.callInFalg && !window.rejectFlag && window.videoChat.group_type){
              ...
              if (this.isLoad==true) {
                this.$nextTick(() => {
                  ...
                  if (this.member.id == window.videoChat.group_id) {
                    window.videoChat = null;
                    window.rejectFlag = null;
                  }
                });
